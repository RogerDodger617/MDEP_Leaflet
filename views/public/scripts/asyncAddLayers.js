...
async function getLayers() {
  ...
  try {
    $.when(
      await $.getJSON('http://localhost:3000/api/Barriers/barrierGeoJSON', function (data) {
        createLayer(data[0].row_to_json, 'Barrier');
        count += 5
      }),

      await $.getJSON('http://localhost:3000/api/DistPoints/distPointGeoJSON', function (data) {
        createLayer(data[0].row_to_json, 'Disturbance Points');
        count += 5
      }),

      await $.getJSON('http://localhost:3000/api/DistLines/distLineGeoJSON', function (data) {
        createLayer(data[0].row_to_json, 'Disturbance Lines');
        count += 5
      }),

      await $.getJSON('http://localhost:3000/api/DistPolygons/distPolyGeoJSON', function (data) {
        createLayer(data[0].row_to_json, 'Disturbance Polygon');
        count += 5
      }),

      $.getJSON('http://localhost:3000/api/DistPolyCentroids/distPolyCentGeoJSON', function (data) {
        createLayer(data[0].row_to_json, 'Disturbance Poly Cent');
        count += 5
      }),

      await $.getJSON('http://localhost:3000/api/RestoPoints/restoPointGeoJSON', function (data) {
        createLayer(data[0].row_to_json, 'Restoration Points');
        count += 5
      }),

      await $.getJSON('http://localhost:3000/api/RestoLines/restoLineGeoJSON', function (data) {
        createLayer(data[0].row_to_json, 'Restoration Lines');
        count += 5
      }),

      $.getJSON('http://localhost:3000/api/Roads/roadsGeoJSON', function (data) {
        createLayer(data[0].row_to_json, 'Roads');
        count += 15
      }),

      $.getJSON('http://localhost:3000/api/SoilVulnerabilities/soilVulnGeoJSON', function (data) {
        createLayer(data[0].row_to_json, 'Soil Vulnerability');
        count += 10
      }),

      await $.getJSON('http://localhost:3000/api/RestoPolygons/restoPolyGeoJSON', function (data) {
        createLayer(data[0].row_to_json, 'Restoration Polygon');
        count += 5
      }),

      $.getJSON('http://localhost:3000/api/RestPolyCentroids/restoPolyCentGeoJSON', function (data) {
        createLayer(data[0].row_to_json, 'Restoration Poly Cent');
        count += 5
      }),

      $.getJSON('http://localhost:3000/api/SnapExtents/snapExtentGeoJSON', function (data) {
        createLayer(data[0].row_to_json, 'Snap Extent');
        count += 5
      }),

      $.getJSON('http://localhost:3000/api/BlmRegions/blmRegionsGeoJSON', function (data) {
        createLayer(data[0].row_to_json, 'BLM');
        count += 5
      }),

      $.getJSON('http://localhost:3000/api/FsRegions/fsRegionsGeoJSON', function (data) {
        createLayer(data[0].row_to_json, 'FS Regions');
        count += 5
      }),

      $.getJSON('http://localhost:3000/api/MdepBoundaries/mdepBoundGeoJSON', function (data) {
        createLayer(data[0].row_to_json, 'MDEP Boundary');
        count += 5
      }),

      $.getJSON('http://localhost:3000/api/MdiBoundaries/mdiBoundGeoJSON', function (data) {
        createLayer(data[0].row_to_json, 'MDI Boundary');
        count += 5
      }),

      $.getJSON('http://localhost:3000/api/NvCounties/nvCountiesGeoJSON', function (data) {
